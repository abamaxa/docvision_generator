...
class ImgAugAugmentor(AbstractAugmentor) :
    ...
    def prepare_frames(self) :
        self.text_boxes = self.convert_frames_to_boxes(self.frames["text"])
        self.question_boxes = self.convert_frames_to_boxes(self.frames["question"])
    ...
    @staticmethod
    def convert_boxes_to_frames(bbs) :
        frames = []   
    ...
    def generate_augmented_image(self) : 
        ...
        if self.frames["text"] :
            text_boxes_aug = seq_det.augment_bounding_boxes([self.text_boxes])[0]
            self.augmented_frames["text"] = ImgAugAugmentor.convert_boxes_to_frames(
                text_boxes_aug)    
            
        if self.frames["question"] :
            question_boxes_aug = seq_det.augment_bounding_boxes([self.question_boxes])[0]
            self.augmented_frames["question"] = ImgAugAugmentor.convert_boxes_to_frames(
                question_boxes_aug)          
